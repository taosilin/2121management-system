  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <div class="text item">
        ...
        <div>
          Carousel图片:
          <el-image :key="item.id" v-for="item in frameDetail.imageList"
            style="width: 100px; height: 100px"
            :src="item"
            fit="contain"></el-image>
        </div>
        <div>
          详情图片:
          <el-image :key="item.id" v-for="item in frameDetail.description"
                    style="width: 100px; height: 100px"
                    :src="item"
                    fit="contain"></el-image>
        </div>
    ...
    <el-dialog class="addSKU" title="添加SKU" :visible.sync="addSKUVisible">
      <el-form
        ...
        <el-form-item label="镜框颜色" >
          <template>
            <el-radio :key="item.id" v-for="(c,i) in colorList" v-model="newSKU.productSpec" :label="c.colorID">
              <el-image
...
export default {
  ...
  created() {
    ...
    axios.post('https://from2121.com:8443/frame/detail',{
      ...
      // 查询当前镜框所有可选镜片
      axios.post('https://from2121.com:8443/framelens/list',{
        frameID: frameID
      }).then(response => {
        for (let i=0;i<response.data.data.length;i++){
      ...
      // 查询全部镜框颜色列表
      axios.post('https://from2121.com:8443/color/list',{
        page: 0,
        size: 20
      }).then(response => {
    ...
    onSave() {
      for (let i=0;i<this.selectedLens.length;i++){
        if (this.originalSelected.indexOf(this.selectedLens[i])==-1){
          axios.post('https://from2121.com:8443/framelens/add',{
      ...
      for (let i=0;i<this.originalSelected.length;i++){
        if (this.selectedLens.indexOf(this.originalSelected[i])==-1){
          axios.post('https://from2121.com:8443/framelens/delete',{
            frameID: this.frameDetail.frameID,
